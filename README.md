...
- **One-line logging** — a single Quick Input box accepts `/win`, `/block` (or `/blocker`), or plain text, and files the entry as the right type automatically.
...
- **Copy Standup to Clipboard** — formats today's wins, blockers, and notes into a Markdown bulleted list, ready to paste into Slack, a PR description, or a standup doc.
- **Dashboard** — a full-screen webview showing your entire history, grouped by day, styled like a terminal log (`+ win`, `- blocker`, `~ note`) using your actual editor font and your color theme's terminal colors.
...
`/win` or `/block`/`/blocker` typed alone with nothing after it is rejected with a warning — it won't log an empty or literal `/win` entry.
...
src/
  ...
  logStore.ts         globalState-backed storage + /win /block parser
